...
# CRITICAL FIX: Model names now include theory substrings (Maxwell, Linearized)
# that match the checks in data.py _compute_theoretical_baseline()
BASELINE_MODELS: Dict[str, Type[torch.nn.Module]] = {
    'Maxwell_Proving_baseline_1': MaxwellBaseline,
    'Linearized_Proving_baseline_2': LinearizedAetherModel,
}

# Define proposed methods
# CRITICAL FIX: Model names now include theory substrings (Lorentz, Dirac)
# that match the checks in data.py _compute_theoretical_baseline()
PROPOSED_MODELS: Dict[str, Type[torch.nn.Module]] = {
    'Lorentz_Proving_proposed': LorentzEtherModel,
    'Dirac_Proving_variant': DiracAetherModel,
}

# Define ablation models
# CRITICAL FIX: Model names now include theory substrings (NoViscosity, Superfluid)
# that match the checks in data.py _compute_theoretical_baseline()
ABLATION_MODELS: Dict[str, Type[torch.nn.Module]] = {
    'NoViscosity_Ablation': NoViscosityAetherModel,
    'Superfluid_Ablation': SuperfluidVacuumModel,
}
...
def print_results_table(results: Dict[str, Dict[str, Dict[str, float]]]) -> None:
    ...
    print("=" * 90)
    print(f"{'Model':<35} {'Primary Metric':<22} {'Secondary Metric':<22} {'Loss':<20}")
    print("-" * 90)
...
def print_comparison_summary(results: Dict[str, Dict[str, Dict[str, float]]]) -> None:
    ...
    if 'Lorentz_Proving_proposed' in results:
        proposed_primary = results['Lorentz_Proving_proposed'].get('primary_metric', {}).get('mean', float('nan'))
    ...
    for baseline_name in ['Maxwell_Proving_baseline_1', 'Linearized_Proving_baseline_2']:
        if baseline_name in results:
            ...
            if not np.isnan(baseline_primary) and abs(baseline_primary) > 1e-10:
                ...
                print(f"Lorentz_Proving_proposed vs {baseline_name}: {improvement:+.2f}% improvement")
            else:
                print(f"Lorentz_Proving_proposed vs {baseline_name}: Unable to compute (baseline=0 or NaN)")
    
    # Compare variant against proposed
    variant_primary = None
    improvement = None
    if 'Dirac_Proving_variant' in results:
        variant_primary = results['Dirac_Proving_variant'].get('primary_metric', {}).get('mean', float('nan'))
        if not np.isnan(variant_primary) and abs(proposed_primary) > 1e-10:
            ...
            print(f"Dirac_Proving_variant vs Lorentz_Proving_proposed: {improvement:+.2f}% difference")
